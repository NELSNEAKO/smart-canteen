const { sequelize } = require('./userModel'); // Ensure a single sequelize instance
const { User } = require('./userModel');
const { Reservation } = require('./reservationModel');
const { ReservationItem } = require('./reservationItemModel');
const { FoodItem } = require('./foodModel');
const { Payment } = require('./paymentModel'); // ✅ Import Payment model
...
Reservation.hasMany(ReservationItem, { as: 'ReservationItems', foreignKey: 'reservation_id', onDelete: 'CASCADE' });
...
User.hasMany(Payment, { as: 'Payments', foreignKey: 'user_id', onDelete: 'SET NULL' });
Payment.belongsTo(User, { as: 'User', foreignKey: 'user_id' });

ReservationItem.hasOne(Payment, { as: 'Payment', foreignKey: 'reservation_item_id', onDelete: 'CASCADE' });
Payment.belongsTo(ReservationItem, { as: 'ReservationItem', foreignKey: 'reservation_item_id' });

// Synchronize all models
sequelize.sync({ alter: true }) // ✅ alter: true updates schema without data loss
